...
class LibAvNotFound(Exception):
	def __repr__(self):
		return "To use ffmpeg video writer, you need to download ffmpeg's library here: https://www.gyan.dev/ffmpeg/builds/. Scroll down to release, and download ffmpeg-release-full-shared.7z, then put all the files from 'bin' folder to osr2mp4 folder (put all files from 'lib' folder to osr2mp4 if you are on macOS)."
