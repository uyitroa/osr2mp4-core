...
class Beatmap:
    def __init__(
        ...
        self.slider_combo = (
            {}
        )  # array of combo that are sliders. to prepare slider frames with those combo
        self.to_stack = []
        self.scale = scale
        self.path = None
        self.hash = None
    ...
    def parse_diff(self):
        ...
        self.diff["ApproachRate"] = self.diff.get(
            "ApproachRate", self.diff["OverallDifficulty"]
        )
        ...
        for line in timings:
            ...
            try:
                ...
                if len(items) < 7 or int(items[6]) == 1:
                    ...
                    timing["BeatDuration"] = (
                        max(10.0, min(1000.0, -float(items[1]))) * inherited / 100
                    )
    ...
    def parse_hitobjecttime(self):
        ...
        for item in hitobjects:
            ...
            self.start_time = int(
                osuobject[2]
            )  # FireRedz: wtf is this on reverse or something
            break
        for item in hitobjects[::-1]:
            ...
            osuobject = item.split(",")
            bin_info = "{0:{fill}8b}".format(
                int(osuobject[3]), fill="0"
            )  # convert int to binary, make it 8-bits
    ...
    def parse_hitobject(self):
        ...
        for item in hitobjects:
            ...
            bin_info = "{0:{fill}8b}".format(
                int(osuobject[3]), fill="0"
            )  # convert int to binary, make it 8-bits # FireRedz: why
            bin_info = bin_info[::-1]  # reverse the binary
            object_type = []
            skip = 0
            ...
            if int(bin_info[2]):
                ...
                if (
                    cur_combo_color not in self.max_combo
                    or cur_combo_number > self.max_combo[cur_combo_color]
                ):
            ...
            if int(bin_info[1]):
                ...
                hitobject["duration"] = (
                    hitobject["BeatDuration"]
                    * hitobject["pixel length"]
                    / (100 * self.diff["SliderMultiplier"])
                )
                hitobject["end time"] = (
                    hitobject["duration"] * hitobject["repeated"] + hitobject["time"]
                )
                ...
                speedmultiplier = (
                    self.timing_point[cur_offset]["Base"] / hitobject["BeatDuration"]
                )
                scoring_distance = 100 * self.diff["SliderMultiplier"] * speedmultiplier
                mindist_fromend = (
                    scoring_distance / self.timing_point[cur_offset]["Base"] * 10
                )
                tickdistance = min(
                    hitobject["pixel length"],
                    max(0, scoring_distance / self.diff["SliderTickRate"]),
                )
                ...
                sliderscoringdistance = (
                    100 * self.diff["SliderMultiplier"]
                ) / self.diff["SliderTickRate"]
                if hitobject["BeatDuration"] > 0:
                    hitobject["velocity"] = (
                        sliderscoringdistance
                        * self.diff["SliderTickRate"]
                        * (1000 / hitobject["BeatDuration"])
                    )
                else:
                    hitobject["velocity"] = (
                        sliderscoringdistance * self.diff["SliderTickRate"]
                    )
            ...
            if (
                len(self.hitobjects) > 0
                and self.hitobjects[-1]["end time"] >= hitobject["time"]
            ):
                self.is2b = True
    ...
    def enddistance(self, obj1, obj2):
        return math.sqrt(
            (obj1["end x"] - obj2["x"]) ** 2 + (obj1["end y"] - obj2["y"]) ** 2
        )
        ...
        for i in range(extendedEndIndex, startIndex, -1):
            ...
            if "circle" in objectI["type"]:
                ...
                while n >= 0:
					...
					"""
                    if (
                        "slider" in objectN["type"]
                        and self.enddistance(objectN, objectI) < stack_distance
                    ):
