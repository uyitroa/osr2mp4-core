...
class Images:
	...
	def __init__(self, filename, scale=1, rotate=0, scales=None):
		...
		if scale != 1:
			self.img_np = self.change_size(scale, scale, np_img=self.img_np)
		if scales is not None:
			for i in scales:
				img = self.change_size(i, i, np_img=self.img_np)
				buf = cl.Buffer(self.ctx, self.mf.READ_WRITE | self.mf.COPY_HOST_PTR, hostbuf=img)
				self.scaledbuffs.append(buf)
	...
	def edit_channel(self, c, scale, buf=None, new_dst=False):
		...
		self.prg.edit_channel(self.queue, (buf.h, buf.w), None, buf.img, dest, buf.w, buf.pix, np.int32(c),
		                      np.float32(scale))
	...
	def change_size(self, row_scale, col_scale, buf=None, np_img=None, scales=None):

		if buf is None and np_img is None:
			buf = self.buf

		if buf is not None:
			np_img, _ = cl.enqueue_map_buffer(queue, buf, cl.map_flags.READ, 0, (buf.h, buf.w, buf.pix),
			                                  np.uint8)

		if scales is not None:
			resizes = []
			for x in scales:
				n_rows = np.int32(max(2, int(x * buf.w)))
				n_rows += np.int32(x % 2 == 1)  # need to be even
				n_cols = np.int32(max(2, int(x * buf.h)))
				n_cols += np.int32(n_cols % 2 == 1)  # need to be even
				resizes.append((cv2.resize(np_img, (x, x), interpolation=cv2.INTER_LINEAR), n_rows, n_cols, buf.pix))
			return resizes
		...
		resized = cv2.resize(np_img, (n_rows, n_cols), interpolation=cv2.INTER_LINEAR)
		dest = cl.Buffer(self.ctx, self.mf.READ_WRITE | self.mf.COPY_HOST_PTR, hostbuf=resized)
	...
	def copy_img(self, buf=None):
		...
		self.prg.copy(self.queue, (buf.h, buf.w, buf.pix), None, buf.img, copy_img, buf.w, buf.w, buf.pix, np.int32(0),
		              np.int32(0))
		return copy_img
	...
	def overlap(self, buf=None, scalar=None, img2=None, x1=None, x2=None, y1=None, y2=None):
		...
		if scalar:
			...
			self.prg.overlap_scalar(self.queue, (y2 - y1, x2 - x1), None, buf.img, buf.w, buf.h, buf.pix, np.int32(x1),
			                        np.int32(y1), r, g, b, a)
		elif img2:
			print("\n\nnot yet\n\n")
	...
	def ensureBGsize(self, bg_buf, overlay_buf):
		if overlay_buf.w > bg_buf.w or overlay_buf.w > bg_buf.h:
			...
			self.prg.copy(self.queue, (bg_buf.h, bg_buf.w, bg_buf.pix), None, bg_buf.img, new_img, bg_buf.w, max_width,
			              bg_buf.pix, x, y)
	...
	def add_to_frame(self, bg_buf, x_offset, y_offset, channel=3, alpha=1, buf=None):
		...
		if channel == 3:
			...
			return
		func(self.queue, (y2 - y1, x2 - x1), None, buf.img, bg_buf.img, buf.w, buf.pix, bg_buf.w, bg_buf.pix, x1, y1,
		     xstart, ystart, np.float32(alpha))
