...
class Images:
	def __init__(self, filename, scale=1, needconversion=False):
		...
		if self.img is None or self.img.size[0] == 1 or self.img.size[1] == 1:
	...
	# 	if overlay_image.size[0] > background.size[0] or overlay_image.size[1] > background.size[1]:
	# 		max_height = max(overlay_image.size[0], background.size[0])
	# 		max_width = max(overlay_image.size[1], background.size[1])
	# 		new_img = np.zeros((max_height, max_width, 4), dtype=overlay_image.dtype)
	# 		y1, y2 = int(new_img.size[0] / 2 - background.size[0] / 2), int(new_img.size[0] / 2 + background.size[0] / 2)
	# 		x1, x2 = int(new_img.size[1] / 2 - background.size[1] / 2), int(new_img.size[1] / 2 + background.size[1] / 2)
	...
	def add_to_frame(self, background, x_offset, y_offset, channel=3):
		# y1, y2 = y_offset - int(self.img.size[0] / 2), y_offset + int(self.img.size[0] / 2)
		# x1, x2 = x_offset - int(self.img.size[1] / 2), x_offset + int(self.img.size[1] / 2)
		#
		...
		background.paste(self.img, (x_offset, y_offset), self.img)
